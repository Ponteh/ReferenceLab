...
std::optional<ReferenceMetadata> LocalReferenceProvider::inspect(const juce::File& f,const juce::File& root,juce::String& error){
    if(!f.existsAsFile()){error="File mancante: "+f.getFullPathName();return{};} if(!supports(f)){error="Formato non supportato: "+f.getFileName();return{};}
    std::unique_ptr<juce::AudioFormatReader> reader(formats.createReaderFor(f)); if(!reader){error="Decodifica fallita: "+f.getFileName();return{};}
...
std::vector<ReferenceMetadata> LocalReferenceProvider::scan(const juce::File& root,bool recursive,juce::String& warning){
    std::vector<ReferenceMetadata> result; if(!root.isDirectory()){warning="Cartella libreria non valida";return result;}
