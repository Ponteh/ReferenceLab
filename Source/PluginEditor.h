...
class ReferenceLabTabs final:public juce::TabbedComponent{public:ReferenceLabTabs():juce::TabbedComponent(juce::TabbedButtonBar::TabsAtTop){}std::function<void(int)>onChange;private:void currentTabChanged(int index,const juce::String&)override{if(onChange)onChange(index);}};
...
private:
    ...
    ReferenceLabAudioProcessor&p;referencelab::AnalysisDisplay analysisDisplay;referencelab::AnalysisMeters analysisMeters;referencelab::ReferenceWaveform waveform;referencelab::EqCurveDisplay eqCurve;ReferenceLabTabs tabs;referencelab::PanelResizer mainSplitter;juce::Component libraryPanel,referencePanel,analysisPanel,comparePanel,detailsPanel;juce::TextButton add{"Aggiungi file"},scan{"Scansiona cartella"},importCatalog{"Importa JSON"},addUrl{"Sorgente web"},remove{"Rimuovi"},relink{"Ricollega"},save{"Salva metadati"},savePreset{"Salva preset"},loadPreset{"Carica preset"},resetMeters{"Reset meter"},newLibrary{"Nuova libreria"},newPlaylist{"Nuova"},addToPlaylist{"+ Playlist"},previousReference{"<"},nextReference{">"},mix{"MIX"},ref{"REFERENCE"},play{"Play"},pause{"Pause"},stop{"Stop"},resetWaveformView{"Vista intera"},fullLoop{"Loop intero"},clearCache{"Svuota cache"};
    juce::Label title,matchedGain,transportStatus,cacheStatus;juce::TextEditor search,filterArtist,filterGenre,filterKey,minBpm,maxBpm,editTitle,editArtist,editAlbum,editGenre,editYear,editBpm,editKey,editRating,editTags,editNotes;juce::ToggleButton recursiveScan{"Sottocartelle"},favouritesOnly{"Solo preferiti"},sortDescending{"Desc."},editFavourite{"Preferita"},autoMatch{"Auto Match"},matchBypass{"Match Bypass"},freezeDisplay{"Freeze"},safeExport{"Safe Export"},loopEnabled{"Loop"},eqBypass{"EQ Bypass"},hpfEnabled{"HPF"},bellEnabled{"Band Pass"},lpfEnabled{"LPF"},transportSync{"Sync DAW"};juce::ComboBox sort,ratingFilter,cacheSize,matchMode,syncMode,listeningMode,hpfSlope,lpfSlope,fftSize,displayRate,mixColourSelector,referenceColourSelector;juce::Slider manualGain,seek,startOffset,loopStart,loopEnd,fade,hpf,bellFreq,bellGain,bellQ,lpf,syncOffset,fftSmoothing;juce::ListBox list{"Reference library",this};
    ...
    std::vector<referencelab::ReferenceMetadata> rows;std::vector<referencelab::ReferencePlaylist> playlistRows;std::vector<referencelab::LibraryDescriptor>libraryRows;juce::ComboBox playlist,librarySelector;juce::Colour appliedMixColour,appliedReferenceColour;double libraryWidthRatio=.34,splitterDragStartRatio=.34;
};
