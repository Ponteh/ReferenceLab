...
struct ComparisonSettings {
    bool bypass=true,highPassEnabled=true,bellEnabled=true,lowPassEnabled=true;
    float highPassHz=20.f,bellHz=1000.f,bellGainDb=0.f,bellQ=1.f,lowPassHz=20000.f;
...
private:
    ...
    struct Channel{Biquad hp1,hp2,bell,lp1,lp2;};struct Chain{Channel channel[2];}mixChain,referenceChain;
    double rate=44100.0;ComparisonSettings settings;ListeningMode previousMode=ListeningMode::stereo,targetMode=ListeningMode::stereo;float modeBlend=1.f,modeStep=0.f,eqWet=0.f,eqStep=0.f;int modeRemaining=0,eqRemaining=0;
    static std::pair<float,float> transformed(float,float,ListeningMode)noexcept;void processModes(juce::AudioBuffer<float>&,juce::AudioBuffer<float>&)noexcept;void processEq(juce::AudioBuffer<float>&,Chain&,float wet)noexcept;void configureFilters()noexcept;
};
}
