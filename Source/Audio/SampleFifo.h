...
public:
    ...
    void push(const juce::AudioBuffer<float>&buffer)noexcept{
        ...
        auto copy=[&](int start,int count,int source){for(int i=0;i<count;++i){const float l=buffer.getSample(0,source+i);const float r=buffer.getNumChannels()>1?buffer.getSample(1,source+i):l;samples[(size_t)(start+i)]=.5f*(l+r);}};
