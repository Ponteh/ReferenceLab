...
HttpDownloadResult HttpDownloader::download(const juce::URL&url,const HttpDownloadOptions&settings,
    ...
    for(int attempt=0;attempt<attempts;++attempt){
        if(cancelled&&cancelled())return failure(ReferenceError::cancelled,"Operazione annullata");
        ...
        if(!input){if(attempt+1<attempts){juce::Thread::sleep(150*(attempt+1));continue;}return failure(ReferenceError::networkUnavailable,"Impossibile connettersi alla "+settings.resourceName,status);}
        if(status<200||status>=300){
            ...
            auto message=cloudflare?"Accesso bloccato dalla verifica Cloudflare per "+settings.resourceName+" (HTTP "+juce::String(status)+")":"Download della "+settings.resourceName+" fallito (HTTP "+juce::String(status)+")";
            return failure(error,message,status);
        }
        auto expected=headers.getValue("Content-Length",{}).getLargeIntValue();
        if(expected>settings.maximumBytes)return failure(ReferenceError::tooLarge,settings.resourceName+" troppo grande",status);
        juce::MemoryOutputStream output;std::array<char,65536>buffer{};
        while(true){
            if(cancelled&&cancelled())return failure(ReferenceError::cancelled,"Operazione annullata",status);
            auto count=input->read(buffer.data(),(int)buffer.size());if(count<=0)break;
            if((juce::int64)output.getDataSize()+count>settings.maximumBytes)return failure(ReferenceError::tooLarge,settings.resourceName+" troppo grande",status);
        ...
        if(output.getDataSize()==0)return failure(ReferenceError::networkUnavailable,settings.resourceName+" vuota",status);
        if(expected>0&&(juce::int64)output.getDataSize()<expected){if(attempt+1<attempts){juce::Thread::sleep(150*(attempt+1));continue;}return failure(ReferenceError::networkUnavailable,"Download della "+settings.resourceName+" incompleto",status);}
    ...
    return failure(ReferenceError::unknown,"Download non riuscito");
}
}
