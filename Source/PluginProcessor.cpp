...
void ReferenceLabAudioProcessor::processBlock(juce::AudioBuffer<float>&mix,juce::MidiBuffer&){
    ...
    referenceBuffer.setSize(mix.getNumChannels(),mix.getNumSamples(),false,false,true); player.process(referenceBuffer);mixAnalysis.process(mix);referenceAnalysis.process(referenceBuffer);auto mixMeters=mixAnalysis.snapshot(),refMeters=referenceAnalysis.snapshot();
    ...
    for(int i=0;i<mix.getNumSamples();++i){auto a=blend.getNextValue();for(int c=0;c<mix.getNumChannels();++c)mix.setSample(c,i,mix.getSample(c,i)*(1.f-a)+referenceBuffer.getSample(c,i)*a);}outputAnalysis.process(mix);
}
