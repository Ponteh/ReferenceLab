...
void ReferenceWaveform::paint(juce::Graphics&g){
    ...
    for(int x=getWidth()-1;x>=0;--x)path.lineTo((float)x,centre+peaks[(size_t)x]*area.getHeight()*.42f);path.closeSubPath();g.setColour(juce::Colour(0xffe99845).withAlpha(.75f));g.fillPath(path);
...
void ReferenceWaveform::mouseDown(const juce::MouseEvent&e){dragStartX=e.x;dragViewStart=viewStart;if(e.mods.isRightButtonDown()||e.mods.isMiddleButtonDown())drag=Drag::pan;else if(loopEnabled&&std::abs(e.x-secondsToX(loopStart))<8)drag=Drag::start;else if(loopEnabled&&std::abs(e.x-secondsToX(loopEnd))<8)drag=Drag::end;else{drag=Drag::seek;auto seconds=xToSeconds((float)e.x);player.seek(seconds);if(onSeek)onSeek(seconds);}}
void ReferenceWaveform::mouseDrag(const juce::MouseEvent&e){if(drag==Drag::pan)viewStart=juce::jlimit(0.0,1.0-viewLength,dragViewStart-(e.x-dragStartX)/(double)juce::jmax(1,getWidth())*viewLength);else if(drag==Drag::start)loopStart=juce::jlimit(0.0,loopEnd-.01,xToSeconds((float)e.x));else if(drag==Drag::end)loopEnd=juce::jlimit(loopStart+.01,duration(),xToSeconds((float)e.x));else{auto seconds=xToSeconds((float)e.x);player.seek(seconds);if(onSeek)onSeek(seconds);}repaint();}
