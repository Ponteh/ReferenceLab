...
void AnalysisDisplay::mouseWheelMove(const juce::MouseEvent&e,const juce::MouseWheelDetails&wheel){
    auto scope=getLocalBounds().toFloat();scope.removeFromTop(getHeight()*.68f);scope=scope.reduced(8);
    if(!scope.contains(e.position)||std::abs(wheel.deltaY)<.001f)return;
    const auto oldLength=scopeViewLength;
    scopeViewLength=juce::jlimit(1.f/256.f,1.f,scopeViewLength*(wheel.deltaY>0.f?.75f:1.333f));
    const auto pointer=juce::jlimit(0.f,1.f,(e.position.x-scope.getX())/juce::jmax(1.f,scope.getWidth()));
    const auto anchor=scopeViewStart+oldLength*pointer;
    scopeViewStart=juce::jlimit(0.f,1.f-scopeViewLength,anchor-scopeViewLength*pointer);
    repaint();
}
...
void AnalysisDisplay::paint(juce::Graphics&g){
    ...
    auto scope=getLocalBounds().toFloat();scope.removeFromTop(getHeight()*.68f);scope=scope.reduced(8);juce::Path wave;for(int x=0;x<(int)scope.getWidth();++x){const auto position=scopeViewStart+scopeViewLength*x/juce::jmax(1.f,scope.getWidth());auto index=juce::jlimit(0,scopeSize-1,(int)(position*(scopeSize-1)));auto y=scope.getCentreY()-outputTime[(size_t)index]*scope.getHeight()*.45f;if(x==0)wave.startNewSubPath(scope.getX(),y);else wave.lineTo(scope.getX()+x,y);}g.setColour(juce::Colours::white.withAlpha(.92f));g.strokePath(wave,juce::PathStrokeType(1));const auto zoomPercent=100.f*defaultScopeViewLength/scopeViewLength;const auto zoomText=zoomPercent<100.f?juce::String(zoomPercent,1):juce::String(juce::roundToInt(zoomPercent));g.setColour(juce::Colours::white.withAlpha(.78f));g.drawText("OUTPUT SCOPE / "+zoomText+"% ZOOM",scope.toNearestInt().removeFromTop(20),juce::Justification::left);paintCursor(g);
}
}
